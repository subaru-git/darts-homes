...
import { BullyBullyResultModel } from '@/db/BullyBullyResultModel';
...
const HistoryTable: FC<HistoryTableProps> = ({ history }) => {
  ...
  return (
    <>
      ...
      {deleteHistory ? (
        <HistoryDeleteAlert
          message={`Bully Bully: ${new Date(deleteHistory.playedAt).toLocaleString('ja-JP', {
            year: 'numeric',
            month: '2-digit',
            day: '2-digit',
            hour: '2-digit',
          ...
          onClose={onClose}
          onDelete={() => {
            deleteFromDB(deleteHistory.id, db.bullyBullyResult);
            onClose();
          }}
        />
      ) : null}
