...
test('should rendering', async () => {
  ...
  for (const _ of Array(2)) {
    await waitFor(() => user.click(findByAriaLabel(buttons, '20 double')));
    await waitFor(() => user.click(findByAriaLabel(buttons, '20 double')));
    await waitFor(() => user.click(findByAriaLabel(buttons, '20 double')));
    await waitFor(() => user.click(findByAriaLabel(buttons, 'round change')));
  }
  expect(screen.getByText('30')).toBeInTheDocument();
  await waitFor(() => user.click(findByAriaLabel(buttons, '20')));
  await waitFor(() => user.click(findByAriaLabel(buttons, '10')));
  await waitFor(() => user.click(findByAriaLabel(buttons, '5 double')));
