...
import { TonsUpResultModel } from '@/db/TonsUpResultModel';
import { db } from '@/db/db';
import { deleteFromDB } from '@/lib/GameHistoryManager/GameHistory';

type HistoryTableProps = {
  history: TonsUpResultModel[];
};

const HistoryTable: FC<HistoryTableProps> = ({ history }) => {
  const [deleteHistory, setDeleteHistory] = useState<TonsUpResultModel | undefined>(undefined);
  ...
  return (
    <>
      ...
      {deleteHistory ? (
        <HistoryDeleteAlert
          message={`Tons Up: ${new Date(deleteHistory.playedAt).toLocaleString('ja-JP', {
            year: 'numeric',
            month: '2-digit',
            day: '2-digit',
            hour: '2-digit',
          ...
          onClose={onClose}
          onDelete={() => {
            deleteFromDB(deleteHistory.id, db.tonsUpResult);
            onClose();
          }}
        />
      ) : null}
