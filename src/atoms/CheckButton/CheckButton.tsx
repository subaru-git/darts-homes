...
const CheckButton: FC<CheckButtonProps & ButtonHTMLAttributes<HTMLButtonElement>> = ({
  disabled = false,
  color = 'green',
  children,
  className,
  ...
  return (
    <button
      className={`${style[c]} w-fit rounded-lg border p-2 text-sm font-bold subpixel-antialiased disabled:cursor-not-allowed md:px-3 md:text-base ${className}`}
      {...props}
      disabled={disabled}
      onClick={(event) => {
